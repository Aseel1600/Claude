...
2. **Real-environment test (when TDD is not possible)** — deploy to the production VPS (`oracle-vps`, see `docs/ops/ORACLE_VPS_OPERATIONS_KB.md`) or the homologation box (`192.168.0.15`, see `docs/ops/HOMOLOGATION.md`) and run a documented live test. Record the exact command + result in the PR description. Applies to: OAuth upstream flows, Cloudflare/WS upstream behavior, UI-only regressions, hardware-dependent behavior.
