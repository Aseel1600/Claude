...
async def memory_graph(request: Request):
    ...
    from orca import kanban, memory_graph as memory_builder, skills as skills_mod
    ...
    graph = memory_builder.build_graph(
        cards=cards, job_list=job_list, skill_defs=skill_defs,
        artifacts=artifacts, agent_file=agent_file, now=now_iso(),
    )
    return no_cache(graph)
