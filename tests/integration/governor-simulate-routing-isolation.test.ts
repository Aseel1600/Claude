...
async function target(mode: "off" | "simulate") { process.env.INTELLIGENCE_GOVERNOR_MODE = mode; const selected: string[] = []; const result = await handleComboChat({ body: { messages: [{ role: "user", content: "small request" }] }, combo: { name: `sim-${mode}`, strategy: "priority", models: ["fixture/provider-model"] }, handleSingleModel: async (_body: unknown, model: string) => { selected.push(model); if (mode === "simulate") GovernorManager.evaluateSimulation({ taskKind: "trivial_control", estimatedPromptTokens: 20, candidates: [{ provider: "fixture", model, tier: "low", available: true, capabilities: [] }], currentModel: model, actualInputTokens: 20, actualOutputTokens: 10, currentCost: null }); return response(); }, isModelAvailable: async () => true, log: { info() {}, warn() {}, error() {}, debug() {} }, settings: null, allCombos: null }); assert.equal(result.ok, true); return selected[0]; }
