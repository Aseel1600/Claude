...
const EXPECTED: Record<InventoryKind, Record<string, number>> = {
  ...
  executor: {
    ...
    "open-sse/services/sessionPool/webExecutorWrapper.ts": 1,
    "src/domain/pipeline.ts": 1,
  ...
  connection: {
    ...
    // #10973 removed two dead existingConnections lookups (4 → 2 actual call sites).
...
const CLASSIFICATION: Record<InventoryKind, Record<string, BypassClass>> = {
  ...
  executor: {
    ...
    "open-sse/services/sessionPool/webExecutorWrapper.ts": "B",
    "src/domain/pipeline.ts": "B",
