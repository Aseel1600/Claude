...
test("mergeModelCompatOverride persists apiFormat, targetFormat, and supportsVision overrides for built-in models", () => {
  ...
  assert.equal((modelOverride as any).apiFormat, "responses");
  assert.equal((modelOverride as any).targetFormat, "claude");
  assert.equal((modelOverride as any).supportsVision, true);
});
