...
describe("migrationRunner/constants — large-table integrity", () => {
  it("RENAMED_MIGRATION_COMPATIBILITY has 30 well-formed entries", () => {
    assert.equal(RENAMED_MIGRATION_COMPATIBILITY.length, 30);
  ...
  it("RENAMED_MIGRATION_COMPATIBILITY spot-checks the boundary renames", () => {
    ...
    assert.deepEqual(
      RENAMED_MIGRATION_COMPATIBILITY.slice(-3).map((entry) => ({
        fromVersion: entry.fromVersion,
        fromName: entry.fromName,
        toVersion: entry.toVersion,
        toName: entry.toName,
      })),
      ["153", "154", "155"].map((fromVersion) => ({
        fromVersion,
