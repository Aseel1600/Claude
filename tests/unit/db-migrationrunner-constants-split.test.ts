...
describe("migrationRunner/constants — large-table integrity", () => {
  ...
  it("RENAMED_MIGRATION_COMPATIBILITY spot-checks the newest 135/136 renumber pair", () => {
    // a from/to transposition on either row would slip past the count + shape checks above
    const newest = RENAMED_MIGRATION_COMPATIBILITY.filter((e) =>
      ["137", "138"].includes(e.toVersion)
    );
