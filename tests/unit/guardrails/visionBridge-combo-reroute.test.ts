...
test("decision: combo with zero vision-capable targets returns 'no-vision'", async () => {
  ...
  assert.equal((await getComboVisionBridgeDecision("text-only-combo")).decision, "no-vision");
});
...
test("decision: combo with all vision-capable targets returns 'skip'", async () => {
  ...
  assert.equal((await getComboVisionBridgeDecision("vision-combo")).decision, "skip");
});
...
test("decision: mixed combo (some vision, some not) returns 'process'", async () => {
  ...
  assert.equal((await getComboVisionBridgeDecision("mixed-combo")).decision, "process");
});

test("decision: unknown model returns 'not-combo'", async () => {
  assert.equal((await getComboVisionBridgeDecision("not-a-combo")).decision, "not-combo");
});
...
test("decision: model-combo mapping routes to the combo decision", async () => {
  ...
  assert.equal((await getComboVisionBridgeDecision("mapped-model-alias")).decision, "no-vision");
});
