...
test("OneMinAiExecutor is registered in the executor index", () => {
  assert.equal(hasSpecializedExecutor("oneminai"), true);
  assert.ok(getExecutor("oneminai") instanceof OneMinAiExecutor);
  assert.equal(hasSpecializedExecutor("1min"), true);
  assert.ok(getExecutor("1min") instanceof OneMinAiExecutor);
});
