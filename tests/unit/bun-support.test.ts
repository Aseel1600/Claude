...
test("createSyncDriverFactory prefers better-sqlite3 when available under Bun", () => {
  const originalBun = process.versions.bun;
  try {
    (process.versions as Record<string, string>).bun = "1.1.20";
...
test("createSyncDriverFactory falls back to bun:sqlite when better-sqlite3 fails under Bun", () => {
  const originalBun = process.versions.bun;
  try {
    (process.versions as Record<string, string>).bun = "1.1.20";
    ...
    const factory = createSyncDriverFactory(loader, () => true);
    const adapter = factory(":memory:");
    assert.ok(adapter, "Adapter should be created via bun:sqlite fallback");
