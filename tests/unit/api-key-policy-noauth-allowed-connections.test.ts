 ...
 * (e.g. OpenCode's Big Pickle model).
 */
...
test("#9057 LAYER1: restricted key gets NO synthetic credentials for noauth provider", async () => {
  ...
  assert.equal(
    creds,
    null,
    "noauth providers must not leak synthetic credentials for a connection-restricted key"
  );
});

test("#9057 LAYER1: unrestricted key still gets synthetic credentials for noauth provider", async () => {
  const creds = await getProviderCredentials(
    "oc",
    null,
  ...
  assert(creds, "unrestricted key must receive synthetic credentials for noauth provider");
  assert.equal(
...
test("#9057 LAYER2: isModelAllowedForKey rejects noauth model for disableNonPublicModels key", async () => {
  ...
  assert.equal(allowed, false, "disableNonPublicModels key must reject noauth models");
});
