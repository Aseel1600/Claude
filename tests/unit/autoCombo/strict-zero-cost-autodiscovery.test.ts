 ...
 *
 * Uses `evaluateStrictZeroCost`'s injectable `catalog` option (added for
 * exactly this test) with synthetic fixtures instead of mutating the real,
 * shared `FREE_MODEL_BUDGETS` — that constant is imported by production code
 * and other test files; mutating it at runtime would be a global side effect
 * this file has no business causing.
 */
...
// `evaluateStrictZeroCost` has exactly one source of truth for "is this
...
test("a brand-new keyless model appearing in the catalog is usable the instant it appears", () => {
  ...
  const candidate = { provider: KEYLESS_PROVIDER, model: modelId };
  ...
  assert.equal(
    evaluateStrictZeroCost(candidate, findBudgetEntry(candidate, catalogAfter), undefined, OPTIONS),
    true,
...
test("a new quota-based model with hardStopGuaranteed + SAFE state is usable automatically", () => {
  ...
  const candidate = { provider: QUOTA_PROVIDER, model: modelId };
  const catalog: FreeModelBudget[] = [quotaEntry(modelId, true)];
  assert.equal(
    evaluateStrictZeroCost(candidate, findBudgetEntry(candidate, catalog), safeState(), OPTIONS),
    true
  );
});
...
test("an entirely unknown provider id is excluded automatically when the default (real) catalog is used", () => {
  const candidate = { provider: "totally-unheard-of-provider-xyz", model: "whatever" };
  assert.equal(
    evaluateStrictZeroCost(candidate, findBudgetEntry(candidate), undefined, OPTIONS),
    false,
