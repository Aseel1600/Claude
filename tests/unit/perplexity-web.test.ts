...
// ─── pplx-opus points at the current Opus generation ────────────────────────

test("MODEL_MAP/THINKING_MAP: pplx-opus resolves to Claude Opus 5", async () => {
  const { MODEL_MAP, THINKING_MAP } =
    await import("../../open-sse/executors/perplexity-web/protocol.ts");

  assert.deepEqual(MODEL_MAP["pplx-opus"], ["search", "claude50opus"]);
