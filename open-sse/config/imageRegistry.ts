...
export const IMAGE_PROVIDERS: Record<string, ImageProviderConfig> = {
  ...
  // Cheaper Inference (https://api.cheaperinference.com) — OSS-sponsor gateway.
  ...
  // ids. parseImageModel() resolves a bare id by first-match over this object's
  // iteration order, so this entry is deliberately declared AFTER adobe-firefly:
  ...
  // IMAGE_MODEL_ALIASES entries for them — that would silently re-route Firefly users.
  //
