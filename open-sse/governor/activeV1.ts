...
export function applyActiveGovernorPlan(request: GovernorMutableRequest, plan: CounterfactualExecutionPlan, correlationId: string, config: ActiveV1Config): { applied: boolean; reason: string; original: GovernorMutableRequest } {
  ...
  const controlled = { ...plan, selectedModel: config.controlModel ? plan.selectedModel : null, selectedProvider: config.controlProvider ? plan.selectedProvider : null, maxOutputTokens: config.controlOutput ? plan.maxOutputTokens : null, reasoningEffort: config.controlReasoning ? plan.reasoningEffort : "preserve", compressionMode: config.controlCompression ? plan.compressionMode : "preserve" };
