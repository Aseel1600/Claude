...
export async function validateFreebuffProvider({ apiKey }: { apiKey: string }) {
  ...
  try {
    ...
    if (res.status >= 500) {
      return { valid: false, error: "Freebuff upstream unavailable", unsupported: false };
    }
    return { valid: false, error: `Freebuff validation failed (${res.status})`, unsupported: false };
  } catch {
    return { valid: false, error: "Freebuff validation network error", unsupported: false };
  }
}
