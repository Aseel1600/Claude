...
export async function getComboVisionBridgeDecision(
  ...
  try {
    ...
    if (!hasModelStep) return { decision: "not-combo", hasConfirmedVision: null };
    // Every model step is proven vision-capable — safe to skip
    if (hasConfirmedVision && !hasNonVisionStep) {
      return { decision: "skip", hasConfirmedVision: true };
    }
    // Mixed combo: some targets lack vision — describe so the combo still answers
    if (hasConfirmedVision) return { decision: "process", hasConfirmedVision: true };
    ...
    return { decision: "no-vision", hasConfirmedVision: false };
  } catch {
...
function extractLastUserText(messages: unknown[]): string | undefined {
  // The deep scan (extractImagePartsFromBody) also routes Responses-API
  // `input`-only bodies (no `messages` list) into the describe path — the
  // task-aware hint must tolerate that.
...
export class VisionBridgeGuardrail extends BaseGuardrail {
  ...
  async preCall(payload: unknown, context: GuardrailContext): Promise<GuardrailResult<unknown>> {
    ...
    // 8. Individual non-combo model with images → optionally REROUTE to best vision-capable model
    ...
    if (rerouteEligible && runtime.mode !== "describe") {
      ...
      if (originalUsable === true && !isAuto && !rerouteTextOnly) {
        ...
        // path below (step 9) which always honors it via getVisionBridgeConfig.
    ...
    // 9. Get configuration — fed from the resolved runtime values so the new
