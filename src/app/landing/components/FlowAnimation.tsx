  ...
  return (
    <div className="mt-16 w-full max-w-4xl overflow-hidden">
      <div className="relative h-[360px] hidden md:flex items-center justify-center animate-[float_6s_ease-in-out_infinite] overflow-hidden">
        ...
        <div className="relative z-20 w-32 h-32 rounded-full bg-[var(--color-bg-alt)] border-2 border-primary shadow-warm flex flex-col items-center justify-center gap-1 group cursor-pointer hover:scale-105 transition-transform duration-500">
          <span className="material-symbols-outlined text-4xl text-rose-gold" aria-hidden="true">
            hub
          </span>
          <span className="text-xs font-bold text-white tracking-widest uppercase">
            {t("brandName")}
          </span>
          <div className="absolute inset-0 rounded-full border border-rose-gold/30 animate-ping opacity-20"></div>
        </div>
        ...
        <div className="absolute left-0 top-1/2 -translate-y-1/2 flex flex-col gap-7">
          {cliTools.map((tool) => (
            ...
            >
              <div className="w-16 h-16 rounded-2xl bg-[var(--color-bg-alt)] border border-border flex items-center justify-center overflow-hidden p-2 hover:border-rose-gold/60 transition-all duration-200 hover:scale-105">
        ...
        >
          <path
            ...
            fill="none"
            stroke={activeFlow === 0 ? "#7F1D1D" : "rgb(75, 85, 99)"}
          ...
          <path
            ...
            fill="none"
            stroke={activeFlow === 1 ? "#7F1D1D" : "rgb(75, 85, 99)"}
          ...
          <path
            ...
            fill="none"
            stroke={activeFlow === 2 ? "#7F1D1D" : "rgb(75, 85, 99)"}
          ...
          <path
            ...
            fill="none"
            stroke={activeFlow === 3 ? "#7F1D1D" : "rgb(75, 85, 99)"}
        ...
        <div className="absolute right-0 top-0 bottom-0 flex flex-col justify-between py-6">
          {providers.map((provider, idx) => (
            <div
              key={provider.id}
              className={`px-3 py-2 rounded-lg ${provider.color} ${provider.textColor} flex items-center justify-center font-bold text-[11px] leading-tight text-center shadow-lg hover:scale-110 transition-all duration-200 cursor-help min-w-[110px] max-w-[140px] ${
                activeFlow === idx ? "ring-4 ring-rose-gold/50 scale-110" : ""
              }`}
              title={provider.name}
            >
              {provider.name}
      ...
      {/* Mobile fallback */}
      <div className="md:hidden mt-8 w-full p-4 rounded-lg bg-[var(--color-bg-alt)] border border-border">
