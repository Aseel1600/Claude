...
async function getAllowedVscodeOwners() {
  const rawConnections = (await getCachedRawProviderConnections()).map(createLazyConnectionView);
  const activeConnections = rawConnections.filter((connection) => connection?.isActive !== false);
  const providerNodes = await getCachedProviderNodes();
  const providerIdToPrefix = new Map<string, string>();
  ...
  for (const node of providerNodes) {
    ...
    if (nodeId && prefix) {
      providerIdToPrefix.set(nodeId, prefix);
    }
  }

  const allowed = new Set<string>();
  for (const connection of activeConnections) {
    const providerId = typeof connection?.provider === "string" ? connection.provider.trim() : "";
    if (!providerId) continue;
    allowed.add(providerId);
    const prefix = providerIdToPrefix.get(providerId);
    if (prefix) {
      allowed.add(prefix);
    }
  }
...
export async function GET(
  ...
  return Response.json(
    (() => {
      const expandedModels = expandVscodeServiceTierModels(activeData);
      ...
      return {
        ...
        data: orderedGroupKeys
          ...
          .filter(Boolean)
          .map((model) =>
            enrichModelForVscode(model as CatalogModelEntry, authorizedRequest, {
