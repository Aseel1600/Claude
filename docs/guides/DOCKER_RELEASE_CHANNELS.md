...
| `:next` / `:next-web` | Active `release/v*` branch | Mutable pre-release pointer | Testing fixes that have landed on the active release branch but are not yet in a stable release |
...
The `next` channel is rebuilt on every push to the active `release/v*` branch and is published for both AMD64 and ARM64. It provides a pullable image for fixes that have merged into the release branch before the next stable tag is cut.
...
`next` is a floating pre-release channel. It may change on any release-branch push and is **not supported for production use**. Pin the image digest while evaluating a specific build:
