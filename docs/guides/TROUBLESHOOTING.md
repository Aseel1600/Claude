...
**Symptom**: When using `model: "auto"` with free/no-auth providers (for example,
`opencode`, `duckduckgo-web`, or `uncloseai`), you intermittently get `HTTP 429`, `400`,
or `401` instead of answers. The requests succeed when retrying the same prompt moments later,
but automation (cron jobs, agents, scripts) breaks on the first failure.

**Root cause**: Three independent failure modes stack up:

1. **Provider rate-limit (`429`)**: Free tiers can enforce an upstream per-window quota. A
   burst of parallel calls exhausts it, so the next request is refused until the window resets.
