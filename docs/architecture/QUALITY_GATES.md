...
| `check:dockerfile-pins`        | Dockerfile base image (`node:22.23.1-bookworm-slim`) and npm global install (`npm@11.19.0`) are pinned — floating `node:`/`npm@latest` tags drift and broke the 2026-08-10 Railway build (better-sqlite3 SIGABRT). Also guarded at build time by a RUN assertion in the Dockerfile. | Yes                                      |
